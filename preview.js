...
window.BooklyPreview = (() => {
  ...
  function sanitizeHtml(value) {
    ...
    temp.querySelectorAll("script, iframe, object, embed, style").forEach(el => {
      el.remove();
    });
      ...
      case "heading":
        return `
          <h2 class="reader-heading">
            ${escapeHtml(block.content || "Sarlavha")}
          </h2>
        `;
      ...
      case "image":
        ...
        return `
          <figure class="reader-image">
            <img
              src="${block.src}"
              alt="${escapeHtml(block.alt || "Kitob rasmi")}"
              loading="lazy"
            />
            ${
              block.caption
    ...
    return `
      <article class="reader-chapter">
        ...
        <h1 class="reader-chapter-title">
          ${escapeHtml(chapter.title || `Bob ${index + 1}`)}
        </h1>
  ...
  function renderAudio(book) {
    if (!book || !book.audio || !book.audio.dataUrl) {
      return "";
    }

    return `
      <div class="reader-audio">
        <div class="reader-audio-title">🎵 Kitob audiosi</div>
        <audio controls preload="metadata">
          <source src="${book.audio.dataUrl}" type="${book.audio.type || "audio/mpeg"}">
  ...
  function renderBook(book) {
    ...
    root.className = `reader theme-${book.theme || "classic"}`;
        ...
        chapters.length
          ...
          : `
            <div class="reader-empty">
              ...
              <p>Bu kitobda hali boblar mavjud emas.</p>
            </div>
          `
      }
  ...
  function open(bookId) {
    ...
    if (!book) {
      console.warn("Preview: kitob topilmadi:", bookId);
      return;
    }
  ...
  function refresh() {
    ...
    const book = BooklyStorage.getBook(currentBookId);
  ...
  function setup() {
    ...
    const exportButton = document.getElementById("preview-export-btn");

    if (exportButton) {
      exportButton.addEventListener("click", () => {
        if (
          window.BooklyExport &&
          typeof window.BooklyExport.exportPDF === "function"
        ) {
          window.BooklyExport.exportPDF(currentBookId);
        }
      });
    }
  }
...
console.log("Bookly Preview tayyor.");
